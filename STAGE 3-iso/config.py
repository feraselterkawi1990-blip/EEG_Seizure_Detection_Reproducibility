...
CHANGELOG (Stage 3-isolated patch - May 2026):
  - FIX:    Wrapped final mkdir loop in try/except to handle
            Python 3.13 FileExistsError on Windows when the
            directory already exists. The bug surfaces only on
            Python 3.13 + Windows because pathlib delegates to
            os.mkdir without honouring exist_ok in some edge cases.
"""
...
# scalp ocular references. (Thesis text S3.2.3 also lists F3/F4/F7/F8;
...
# Variance-threshold feature filter (S3.3.4 of thesis Table 3.1).
...
#                    Matches thesis S3.5.3 (~1:5 ictal:non-ictal ratio).
...
# Test set is always balanced (50/50) per held-out patient (S3.5.2).
TEST_BALANCE = True
...
# =====================================================================
# Internal validation split for threshold tuning (S3.5.4 of thesis).
...
# Threshold search (S3.5.4 of thesis Table 3.1).
...
# =====================================================================
# Wrapped in try/except to handle a Python 3.13 + Windows quirk where
# Path.mkdir(exist_ok=True) can still raise FileExistsError if the
# directory was created in a previous interpreter session. Functionally
# equivalent to the original mkdir(parents=True, exist_ok=True) but safe
